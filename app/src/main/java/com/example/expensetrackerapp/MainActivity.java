                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (e instanceof FirebaseAuthInvalidCredentialsException) {
                                ...
                                // Network error
                                Toast.makeText(MainActivity.this, "Authentication failed. Network error.",
                                        Toast.LENGTH_SHORT).show();
                            } else {
                                // Other errors
                                Log.w(FIREBASE_AUTHENTICATION_LOG_TAG, "signInWithEmail:failure", task.getException());
                                Toast.makeText(MainActivity.this, "Authentication failed. Try again later.",
                                        Toast.LENGTH_SHORT).show();
                            }
                            //updateUI(null);
                        }
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (e instanceof FirebaseAuthWeakPasswordException) {
                                // weak password
                                Toast.makeText(MainActivity.this, "Authentication failed. Password should be at least 6 characters", Toast.LENGTH_SHORT).show();
                            }
                            else if (e instanceof FirebaseAuthUserCollisionException) {
                                // email already in use
                                Toast.makeText(MainActivity.this, "Authentication failed. Email already in use", Toast.LENGTH_SHORT).show();
                            }
                            else if (e instanceof FirebaseNetworkException) {
                                // network error
                                Toast.makeText(MainActivity.this, "Authentication failed. Network error", Toast.LENGTH_SHORT).show();
                            }
                            else {
